...
class ActorCritic(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print(act_mask)
        # print(actor_logits)
        # inverse_act_mask = 1-act_mask
        # inverse_act_mask = [m == 1 for m in inverse_act_mask]
        # [-999999.0 if inverse_act_mask(i) else actor_logits(i) for i in range(len(inverse_act_mask))]
        # print(inverse_act_mask)
        # [print(m) for m in act_mask]
        # [print(l) for l in actor_logits]

        for j in range(len(act_mask)):
            for i in range(len(act_mask[j])):
                if act_mask[j, i] == 0:
                    actor_logits[j, i] = -999999.0
